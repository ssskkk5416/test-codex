...
def load_tabular_file(uploaded_file: io.BytesIO) -> Tuple[pd.DataFrame, List[str]]:
...
uploaded = st.file_uploader("犯罪データファイル (CSV / Excel)", type=["csv", "xlsx", "xls"])
...
with lower_left:
    ...
    if not ward_summary.empty:
        ...
        for metric_col, (_, ward_row) in zip(metric_cols, top_wards_df.iterrows()):
            metric_col.metric(str(ward_row[ward_col]), f"{ward_row['件数']}件")

with upper_right:
    st.subheader("概要")
    if summary_col:
        summaries = filtered_df.dropna(subset=[summary_col]) if not filtered_df.empty else pd.DataFrame()
        if summaries.empty:
            st.info("該当する概要がありません。条件を変更してください。")
            selected_summary_value = None
        else:
            summary_choices = summaries[summary_col].dropna().unique().tolist()
